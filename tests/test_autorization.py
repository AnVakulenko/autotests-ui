...
def test_wrong_email_or_password_autorization():
    with sync_playwright() as playwright:
        ...
        page.goto(
            "https://nikita-filonov.github.io/qa-automation-engineer-ui-course/#/auth/login")  # переход на страницу по ссылке
        ...
        password_input = page.get_by_test_id('login-form-password-input').locator(
            'input')  # найдет поле с таким локатором
        ...
        expect(wrong_email_or_password_alert).to_have_text(
            'Wrong email or password')  # проверяет что отображается именно этот текст
